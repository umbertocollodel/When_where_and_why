...
list(comparison_wb,comparison_ec,comparison_consensus) %>% 
  map(~ .x %>% mutate_at(vars(matches("wb\\d|ec\\d|consensus\\d")),funs(targety_first - .))) %>% 
  map(~ .x %>% mutate(recession = case_when(targety_first <= 0 ~ "Recession",
                                            T ~ "Non-recession"))) %>% 
  map(~ .x %>% select(country_code,year,matches("wb\\d|ec\\d|consensus\\d"),recession)) %>% 
  map(~ .x %>% rename_at(vars(matches("consensus")),funs(str_replace(.,"consensus","cn")))) %>% 
  reduce(merge, all=T) %>% 
  gather("institution","fe",wb1:ncol(.)) %>%
